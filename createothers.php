...
if(isset($_POST['save'])){
	...
	$mysqli->query("INSERT INTO others(idnumber, lastname, firstname, middlename, gender, company, telephone)
	VALUES('$idnumber', '$lastname', '$firstname', '$middlename','$gender','$company', '$telephone')") or
	 die($mysqli->error);
...
if (isset($_GET['delete'])){
	...
	$mysqli->query("DELETE FROM others WHERE idnumber=$idnumber") or die($mysqli->error());
...
if (isset($_GET['edit'])){
	$idnumber = $_GET['edit'];
	$update = true;
	$result = $mysqli->query("SELECT * FROM others WHERE idnumber=$idnumber") or die($mysqli->error());
...
if (isset($_POST['update'])){
	...
	$mysqli->query("UPDATE others SET idnumber='$idnumber', lastname='$lastname', firstname='$firstname', middlename='$middlename', gender='$gender', company='$company', telephone='$telephone' WHERE idnumber=$idnumber") or
	die($mysqli->error);
	$_SESSION['message'] = "Record has been updated!";
	$_SESSION['msg_type'] = "success";
 	        ?>
	        <script type="text/javascript">
            alert("Others Updated.");
            window.history.go(-2);
            </script>
            <?php
            }?>
...
Add Others Information
  ...
  <div class="card-body">
<small>
<form action="createothers.php" role="form" method="POST">
<div class="row">
...
<div class="row">     
      ...
      <div class="form-group">
	    <input type="text" name="company" class="form-control" value="<?php echo $company; ?>" placeholder="Company" required>
	  </div>
      </div>	  
